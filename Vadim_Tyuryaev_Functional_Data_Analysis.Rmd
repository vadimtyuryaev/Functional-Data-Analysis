...
- *"Functional data analysis (FDA), a repertoire of statistical methods that considers data as evaluations of curves (mathematical functions) over a discrete grid"* [@XU2020337].  \vspace{0.3cm}

- Rather than being a scalar or a vector, each observation $i$ is a function, $x_i(t)$, defined over a domain $\mathcal{T} \subset \mathbb{R}$.\vspace{0.3cm}

- A critical assumption in this context is the **smoothness** of the underlying function, $x(t)$. Should this smoothness condition not hold, adopting a functional data framework over a conventional multivariate analysis is not sensible [@Ramsay2005].\vspace{0.3cm}

- Many FDA methods are alterations of classical multivariate methods such as principal components analysis (PCA) and linear models [@muller2022functional;@ramsay2001fda]. 

## Slide 1b: Goals and Distinctions of FDA

- The goals [@Ramsay2005] of functional data analysis align with classical statistical objectives:
  ...
  - Model and explain variation in a response using functional predictors or covariates.\vspace{0.3cm}

- FDA relies on:
  - Hilbert space theory.
  - Basis function expansions (e.g., 
  [Generalized Fourier Series](https://mathworld.wolfram.com/GeneralizedFourierSeries.html),
  [B-splines](https://personal.math.vt.edu/embree/math5466/lecture10.pdf)).
  - [Regularization](https://www.ibm.com/think/topics/regularization) techniques to prevent overfitting.

## Slide 2a: Hilbert Space and Fourier Series

- A **Hilbert space** [@Blanchard2015], denoted as $\mathcal{H}$, is a real or complex vector space that is also a complete inner product space.\vspace{0.3cm}
...
- Key concepts:
  ...
  - **Generalized Fourier Series**: $f=\sum_{k=0}^{\infty}c_k\phi_k$ and $c_k =\langle f, \phi_k \rangle$ \vspace{0.3cm}
...
- The above is based on the following [integral identities](https://mathworld.wolfram.com/FourierSeries.html) [@mathworldFourierSeries]:
\[
\begin{aligned}
\int_{-\pi}^{\pi} \sin(mt)\sin(nt)\, dt &= \pi \delta_{mn}, \quad \int_{-\pi}^{\pi} \cos(mt)\cos(nt)\, dt = \pi \delta_{mn} \\
\int_{-\pi}^{\pi} \sin(mt)\cos(nt)\, dt &= 0, \quad \quad \int_{-\pi}^{\pi} \sin(mt)\, dt = 0, \quad \int_{-\pi}^{\pi} \cos(mt)\, dt = 0
\end{aligned}
\]
...
- Zeroth coefficient:
  ...
  = \frac{1}{\pi} \left[ \int_{-\pi}^{\pi} t\,dt + \int_{-\pi}^{\pi} \cos(t)\,dt \right] = 0
  \] \vspace{0.3cm}

- First cosine coefficient:
  \[
  \begin{aligned}
a_1 = \frac{1}{\pi} \int_{-\pi}^{\pi} f(t)\cos(t)\,dt = \frac{1}{\pi} \left[ \int_{-\pi}^{\pi} t\cos(t)\,dt + \int_{-\pi}^{\pi} \cos^2(t)\,dt \right] &= \\
...
\[
 \begin{aligned}
b_1 = \frac{1}{\pi} \left[ \int_{-\pi}^{\pi} t\sin(t)\,dt + \int_{-\pi}^{\pi} \cos(t)\sin(t)\,dt \right] &= \\
= \frac{1}{\pi} \left[ 2\pi + 0 \right] \ &= 2
  \end{aligned}
\]
...
\frac{1}{\pi} \left[ \int_{-\pi}^{\pi} t\cos(2t)\,dt + \int_{-\pi}^{\pi} \cos(t)\cos(2t)\,dt \right] \
...
\[
\begin{aligned}
b_2 = \frac{1}{\pi} \int_{-\pi}^{\pi} f(t)\sin(2t)\,dt &= \\
\frac{1}{\pi} \left[ \int_{-\pi}^{\pi} t\sin(2t)\,dt + \int_{-\pi}^{\pi} \cos(t)\sin(2t)\,dt \right] \
= \frac{1}{\pi} \left[ -\pi + 0 \right] &= -1
\end{aligned}
\]

## Slide 2g (Example): Computational Approximation of $f(t)$

```{r, echo=FALSE,fig.width=4.75, fig.height=3.25,  fig.align='center', warning=FALSE, message=FALSE}
...
- Given $n+1$ distinct interpolation points $t_0, t_1, \dots, t_n$ and function values $y_0, y_1, \dots, y_n$, the **Lagrange interpolation polynomial** $P_n(t)$ is defined by [@Walter2011]:
  \[
...
# Function to evaluate the Lagrange interpolating polynomial
eval_lagrange = function(x, x_vals, y_vals) {
  sapply(x, function(x0) {
    sum(sapply(1:length(x_vals), function(i)
      y_vals[i] * lagrange_basis(x0, i, x_vals)))
...
into a series of subintervals:
\[
a = x_1 < x_2 < x_3 < \dots < x_{n-1} < x_n = b
\]

- The function is approximated on each subinterval $[x_i, x_{i+1}]$ (for $i = 1, 2, \dots, n-1$) 
using a separate, **low-degree polynomial**.

## Slide 4a:  Theory

- A **B-spline** [@Walter2011;@patrikalakis2000section14] is a piecewise polynomial with local support, defined over knots $\{\tau_i\}$.
...
generate_basis_plot = function(degree, knots, show_legend = FALSE) {
  ...
  basis_matrix = spline.des(knots, x_vals, k, derivs = rep(0, length(x_vals)), outer.ok = TRUE)$design
  ...
  if (degree == 0) {
    ...
    label_data = data.frame(
      ...
      final_y = 1.5,
      label_text = c("B[list(2, 1)]", "B[list(3, 1)]", "B[list(4, 1)]", "B[list(5, 1)]")
    )
  } else if (degree == 2) {
    label_data = data.frame(
      x_pos = c(-2, -1, 0, 1, 2),
      final_y = 1.5,
      label_text = c("B[list(1, 2)]", "B[list(2, 2)]", "B[list(3, 2)]", "B[list(4, 2)]", "B[list(5, 2)]")
    )
  }
  ...
  p = ggplot(plot_data_final, aes(x = x, y = y, group = basis_id)) +
    ...
    geom_line(aes(linetype = linetype_style, color = color_style), linewidth = 1.2, na.rm = TRUE, show.legend = FALSE) +
    ...
    # ***** THIS IS THE CORRECTED LAYER *****
    # Added inherit.aes = FALSE to prevent the error
    geom_text(
      data = label_data, aes(x = x_pos, y = final_y, label = label_text),
      parse = TRUE, size = 4.5, color = "black", show.legend = FALSE, inherit.aes = FALSE
    ) +
    
    geom_segment(
      aes(x = support_domain[1], xend = support_domain[2], y = -0.05, yend = -0.05),
    ...
    scale_x_continuous(limits = x_range, breaks = seq(x_range[1], x_range[2], by = 1)) +
    labs(y = "", x = "") +
    theme_classic() +
...
# grid.arrange(
#   arrangeGrob(plot_d0, plot_d1, plot_d2, ncol = 1),
#   shared_legend,
#   ncol = 2,
...
```{r, echo=FALSE,fig.width=4.5, fig.height=3.25,  fig.align='center', warning=FALSE, message=FALSE}
...
## Slide 4m (Concept): A Clamped Quadratic B-spline {.t}

<!-- A B-spline curve (degree $p$) is a piecewise-polynomial curve defined by control points and a knot vector. It follows (but not interpolates) its interior control points interpolating the endpoints if the knot vector is clamped. Changing a single control point affects the curve only over the support of its basis function—at most $p\!+\!1$ neighboring knot spans.  -->

```{r,echo=FALSE, fig.width=4.75, fig.height = 4.75, fig.align='center', warning=FALSE, message=FALSE,dev='pdf', fig.crop=FALSE}
...
ggplot() +
  ...
  geom_path(data = control_points, aes(x = x, y = y), linetype = "dashed", color = "gray40") +
  ...
  # Add a title
  #labs(title = "Clamped Quadratic B-Spline Curve") +
...
# Data frames
df_true = data.frame(t = t_full, y = y_true, type = factor("f(t)", levels = c("f(t)", "B-spline")))
df_spline = data.frame(t = t_full, y = y_smooth, type = factor("B-spline", levels = c("f(t)", "B-spline")))
...
df_true = data.frame(t = t_full, y = y_true, type = factor("f(t)", levels = c("f(t)", "B-spline")))
df_spline = data.frame(t = t_full, y = y_smooth, type = factor("B-spline", levels = c("f(t)", "B-spline")))
df_points = data.frame(t = t_sample, y = y_sample, type = factor("Sample", levels = c("Sample", "Extrapolated")))
df_extrap = data.frame(t = x_extrap, y = y_extrap, type = factor("Extrapolated", levels = c("Sample", "Extrapolated")))
...
- **Algorithm** [@dash2003fast;@ferreira2009comparison]: 
  - Start with every data point (curve) in its own cluster.
  - Find the two "closest" clusters and merge them.
  - Repeat until all data points are in a single cluster.\vspace{0.3cm}
...
continuous function using an appropriate smoothing technique and the integration is performed on the analysitcal expression. 
...
true_L2_squared = integrate(function(t) (f(t) - g(t))^2, lower = 0, upper = 2*pi)$value
...
knitr::kable(results, "latex", booktabs = TRUE, caption = "Convergence of Trapezoid Rule Approximation of L2 Distance")
...
calculate_L2_for_nbasis = function(nb, argvals, y1, y2) {
  ...
  bspline_basis = create.bspline.basis(rangeval = c(a, b), nbasis = nb, norder = 4)
...
knitr::kable(results, "latex", booktabs = TRUE, caption = "Convergence of B-Spline Approximation of L2 distance")
...
- Rand Index [@rand1971objective]:
  $$ \text{RI} = \frac{TP + TN}{TP + TN + FP + FN} \in [0,1] $$
  
- Adjusted Rand Index (ARI) [@hubert1985comparing]:  
...
calculate_rand_index = function(true_labels, predicted_labels) {
  ...
  # a: Number of pairs that are in the SAME cluster in both partitions (True Positives)
  # b: Number of pairs that are in DIFFERENT clusters in both partitions (True Negatives)
...
gics_sectors = tribble(
  ...
  "HD", "Consumer Discretionary", "HON", "Industrials", "IBM", "Information Technology", 
  "JNJ", "Health Care", "KO", "Consumer Staples", "JPM", "Financials", 
  "MCD", "Consumer Discretionary", "MMM", "Industrials", "MRK", "Health Care", 
  "MSFT", "Information Technology", "NKE", "Consumer Discretionary", "PG", "Consumer Staples", 
  "SHW", "Materials", "TRV", "Financials", "UNH", "Health Care", 
  "CRM", "Information Technology", "NVDA", "Information Technology", "VZ", "Communication Services", 
  "V", "Financials", "WMT", "Consumer Staples", "DIS", "Communication Services"
)
...
# ***DEFINITIVE FIX***: Find the intersection of stocks with BOTH complete price data AND sector data
final_valid_stocks = intersect(stocks_with_prices, stocks_with_sectors)

# ***FAILSAFE***: Add a check to stop if there are not enough valid stocks to proceed.
...
# ***FAILSAFE***: Add a check to stop if there are not enough common dates.
...
# ***FAILSAFE***: Stop if any NA values were created during label generation
...
#cat("Found", num_sectors, "unique GICS sectors in the final sample.\n")
#cat("This will be the number of clusters (k) we aim for.\n\n")
...
for(i in 1:n_stocks) {
  for(j in 1:i) {
    # d(i,j)^2 = inprod(i,i) + inprod(j,j) - 2*inprod(i,j)
...
# cat(" Clustering Performance Evaluation \n")
# cat("Rand Index (RI):", round(calculate_rand_index(true_group_labels,predicted_group_labels), 4), "\n") # custom
# cat("Adjusted Rand Index (ARI):", round(cluster_comparison["ARI"], 4), "\n\n")                          # from flexclust
# cat("Interpretation:\n")
# cat("- Rand Index: The probability that a random pair of stocks is classified correctly (either together in both, or separate in both).\n")
# cat("- Adjusted Rand Index: Ranges from -1 to 1. 1 means perfect agreement. 0 means the clustering is no better than random chance. Negative values are worse than random.\n")

```

## Slide 5g: DJIA by Sector before HAC

```{r,echo=FALSE,fig.width=4.5, fig.height=3,  fig.align='center', warning=FALSE, message=FALSE}
...
ggplot(plot_df_by_sector, aes(x = time, y = normalized.price, group = symbol, color = sector)) +
  geom_line(alpha = 0.7) +
...
```{r, echo=FALSE,fig.width=5, fig.height=4.75,  fig.align='center', warning=FALSE, message=FALSE}
...
plot_df = data.frame(
  ...
  pivot_longer(-time, names_to = "symbol", values_to = "normalized.price") %>%
  left_join(data.frame(symbol = names(predicted_group_labels), cluster = predicted_group_labels), by = "symbol")

ggplot(plot_df, aes(x = time, y = normalized.price, group = symbol, color = as.factor(cluster))) +
  geom_line(alpha = 0.7) +
...
* **Structural Parameters ($\mathbf{\theta}$):** These are the parameters of primary interest within the differential equation, such as reaction rates or physical constants.
...
The main challenge is to estimate the structural parameters $\mathbf{\theta}$ accurately, without them being overwhelmed by the potentially vast number of nuisance parameters $\mathbf{c}$.
...
Instead of a joint optimization over both $\mathbf{\theta}$ and $\mathbf{c}$, the method uses a hierarchical, multi-criterion approach called **generalized profiling** or a **parameter cascade** [@ramsay2007parameter].
...
The goal is to estimate parameters for the **FitzHugh-Nagumo** equations [@Postnikov2016], which model the electrical potential across a neuron's membrane.
...
![The FitzHugh-Nagumo Model Solution via Generalized Profiling](C:/Users\vadim/Documents/FDA/FitzHugh-Nagumo-ODE_Bsplines.jpg)
...
Runge-Kutta (RK) methods [@shampine1986practical] are a family of algorithms that dramatically improve accuracy by using a **weighted average of multiple slopes** within each step.
...
Popular MATLAB's solver [`ode45`](https://www.mathworks.com/help/matlab/ref/ode45.html) relies on **Runge-Kutta 4(5)** method which is also known as the **Dormand-Prince (4,5) pair**.  The slopes are calculated in the following manner [@shampine1986practical]: 
...
$$
  \begin{aligned}
  y_{n+1}^{(4)} = y_n + h \Biggl( &\frac{5179}{57600}k_1 + \frac{7571}{16695}k_3 + \frac{393}{640}k_4 \\
                                 & - \frac{92097}{339200}k_5 + \frac{187}{2100}k_6 + \frac{1}{40}k_7 \Biggr)
  \end{aligned}
$$
...
## Slide A1f (Example): Linear Equations $y_1^{\prime} = y_2$ and $y_2^{\prime} = -y_1$ 
...
p_static = ggplot() +
  ...
  geom_path(data = true_df_fine, aes(x = y1_true, y = y2_true), color = "black", size = 1) +
  # Plot the RK4(5) solution path as a blue dashed line
  geom_path(data = solution_df, aes(x = y1, y = y2), color = "blue", linetype = "dashed") +
  # Plot the actual RK4(5) steps as red crosses
  geom_point(data = solution_df, aes(x = y1, y = y2), color = "red", shape = 4, size = 3, stroke = 1.2) +
  labs(
...
## Slide A1g (Example): Non-linear Equations {.fragile}
...
solve_ode_rk45 = function(f, t_span, y0, h0 = 0.1,
                           abs_tol = 1e-6, rel_tol = 1e-3, trace = FALSE, trace_steps = 3) {
...
# - Renders a formatted table for either LaTeX or HTML -
 ...
 render_trace_table = function(trace_df, format = "html") {
   ...
   kableExtra::add_header_above(
       c(" " = 3, "Stage Slopes" = 7, "Predictions" = 2, "Control" = 3, "Truth" = 1)
       )
...
The **Levenberg-Marquardt (LM) algorithm** [@more1978levenberg] introduces a regularization or damping parameter $\lambda \ge 0$ to ensure stability:
...
gg_initial = ggplot(data_df, aes(x = x, y = y)) +
  ...
  stat_function(fun = model_func, args = list(beta = beta_init), aes(color = "Initial Guess"), size = 1.2) +
  labs(title = "The Problem: Data and Initial Guess",
       subtitle = paste0("Initial Beta = (", round(beta_init[1], 2), ", ", round(beta_init[2], 2), ")"),
...
gg_ssr_surface = ggplot(ssr_grid, aes(x = beta1, y = beta2)) +
  ...
  annotate("point", x = beta_true[1], y = beta_true[2], color = "cyan", size = 5, shape = 4, stroke = 1.5) +
  annotate("point", x = beta_init[1], y = beta_init[2], color = "red", size = 4, shape = 18) +
  annotate("text", x = beta_true[1], y = beta_true[2] + 0.2, label = "True Minimum", color = "cyan", size = 4) +
  annotate("text", x = beta_init[1], y = beta_init[2] - 0.1, label = "Initial Guess", color = "red", size = 4) +
...
beta_k = beta_init
lambda = 10
history = data.frame(beta1 = beta_k[1], beta2 = beta_k[2], ssr = ssr_func(beta_k, x_data, y_data))
...
perform_lm_step = function(beta_k, lambda, history) {
  ...
  beta_h1 = beta_k; beta_h1[1] = beta_h1[1] + h; r_h1 = y_data - model_func(beta_h1, x_data)
  # FIXED: Corrected the sign for the Jacobian calculation
  J[, 1] = (r_h1 - residuals_k) / h
  
  # Perturb beta2 and calculate derivative
  beta_h2 = beta_k; beta_h2[2] = beta_h2[2] + h; r_h2 = y_data - model_func(beta_h2, x_data)
  # FIXED: Corrected the sign for the Jacobian calculation
  ...
  return(list(beta_k = beta_k, lambda = lambda, history = history, delta_beta = delta_beta))
}
...
plot_lm_iteration = function(k, beta_k, lambda, history, gg_ssr_surface) {
  ...
  gg_fit_k = ggplot(data_df, aes(x = x, y = y)) +
    ...
    stat_function(fun = model_func, args = list(beta = beta_true), aes(color = "True"), size = 1.2, linetype = "dashed") +
    stat_function(fun = model_func, args = list(beta = beta_k), aes(color = "Current Fit"), size = 1.2) +
    labs(title = paste("Iteration", k, ": Model Fit"), subtitle = paste0("SSR = ", round(ssr_val, 3)), x = "x", y = "y") +
    scale_color_manual(name = "Model", values = c("Current Fit" = "blue", "True" = "cyan")) +
  ...
  gg_path_k = gg_ssr_surface + 
    geom_path(data = history, aes(x = beta1, y = beta2), color = "orange", size = 1.1, arrow = arrow(length = unit(0.25, "cm"), type = "closed")) +
    geom_point(data = history, aes(x = beta1, y = beta2), color = "red", size = 3) +
    labs(title = paste("Iteration", k, ": Path on SSR"), subtitle = paste0("Current Lambda = ", lambda))+
    theme(plot.title = element_text(hjust = 0.5), 
          plot.subtitle = element_text(hjust = 0.5),
          axis.title = element_text(size = 7),
          axis.text = element_text(size = 7),
...
gg_final = ggplot(data_df, aes(x = x, y = y)) +
  geom_point(color = "black", size = 3, alpha = 0.8, shape = 16) +
  stat_function(fun = model_func, args = list(beta = beta_k), aes(color = "Final LM Fit"), size = 1.2) +
  stat_function(fun = model_func, args = list(beta = beta_true), aes(color = "True Model"), size = 1.2, linetype = "dashed") +
  labs(title = "Final Result: Levenberg-Marquardt Fit",
       subtitle = paste0("Converged in ", k, " iterations to Beta = (", round(beta_k[1], 3), ", ", round(beta_k[2], 3), ")"),
       x = "x (e.g., Days)", y = "y (e.g., Cases)") +
  scale_color_manual(name = "Model", values = c("Final LM Fit" = "darkgreen", "True Model" = "cyan")) +
  theme_bw(base_size = 10) + #14
  theme(legend.position = "bottom")+
  theme(plot.title = element_text(hjust = 0.5), 
        plot.subtitle = element_text(hjust = 0.5), 
